...
export function CandidateList() {
  ...
  const handleCreate = async (data: Omit<Candidate, 'id' | 'created_at' | 'updated_at'>) => {
    const newCandidate = await candidatesService.create({ ...data, created_by: user?.id || null });
    ...
    <div className="p-8">
      ...
      {candidates.length === 0 ? (
        ...
        <PipelineView
          ...
          onCandidateClick={(c) => navigate(`/candidates/${c.id}`)}
          onEdit={(c) => { setSelectedCandidate(c); setModalOpen(true); }}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                    {candidate.email && (
                      <a href={`mailto:${candidate.email}`} onClick={e => e.stopPropagation()} className="flex items-center gap-1 hover:text-gray-900">
                        <Mail className="w-4 h-4" />{candidate.email}
                      </a>
                    )}
                    {candidate.phone && (
                      <a href={`tel:${candidate.phone}`} onClick={e => e.stopPropagation()} className="flex items-center gap-1 hover:text-gray-900">
                        <Phone className="w-4 h-4" />{candidate.phone}
                      </a>
                    )}
                    {candidate.linkedin_url && (
                      <a href={candidate.linkedin_url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="flex items-center gap-1 hover:text-gray-900">
                ...
                <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                  <button
                    onClick={() => { setSelectedCandidate(candidate); setModalOpen(true); }}
...
export function CandidateDetail() {
  ...
  const handleUpdate = async (data: Omit<Candidate, 'id' | 'created_at' | 'updated_at'>) => {
    if (!candidate) return;
    const updated = await candidatesService.update(candidate.id, data);
    ...
    try {
      if (user) {
        await activitiesService.createStageMove(
          ...
          candidate.status ?? 'New', newStatus, user.id
        );
      }
      const updated = await candidatesService.updateStatus(candidate.id, newStatus);
      setCandidate(updated);
    } finally {
  ...
  return (
    <div className="p-8">
      <button onClick={() => navigate('/candidates')} className="mb-6 text-blue-600 hover:text-blue-700 font-medium">
        ← Back to Candidates
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <h1 className="text-xl font-bold text-gray-900 mb-3">{candidate.full_name}</h1>
            ...
            <div className="space-y-3 text-sm">
              {candidate.email && (
                <div>
                  ...
                  <a href={`mailto:${candidate.email}`} className="text-blue-600 hover:underline break-all">{candidate.email}</a>
                </div>
              )}
              {candidate.phone && (
                <div>
              ...
              {candidate.linkedin_url && (
                <div>
                  ...
                  <a href={candidate.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Profile</a>
                </div>
              )}
              {candidate.resume_url && (
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Resume</p>
                  <a href={candidate.resume_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Download</a>
                </div>
              )}
            </div>
          </div>
